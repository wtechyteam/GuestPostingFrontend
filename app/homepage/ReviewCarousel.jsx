...
export default function ReviewCardsContainer({ reviews = [] }) {
  const [sliderRef, slider] = useKeenSlider({
    ...
    loop: true,
    duration: 500, // Duration of slide animation in milliseconds
  });

  useEffect(() => {
    if (!slider) return; // Ensure the slider is initialized
  ...
  }, [slider]); // Depend on slider to re-run effect when it's initialized
